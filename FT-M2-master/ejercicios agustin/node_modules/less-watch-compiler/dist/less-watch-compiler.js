...
program.version(packagejson.version, '-v, -V, --version', 'Output the current version').usage('[options] <source_dir> <destination_dir> [main_file_name]').option('--main-file <file>', "Specify <file> as the file to always re-compile e.g. '--main-file style.less'.").option('--config <file>', 'Custom configuration file path.', 'less-watch-compiler.config.json').option('--run-once', 'Run the compiler once without waiting for additional changes.').option('--include-hidden', "Don't ignore files beginning with a '.' or a '_'")
//Less Options
.option('--enable-js', 'Less.js Option: To enable inline JavaScript in less files.').option('--source-map', "Less.js Option: To generate source map for css files.").option('--plugins <plugin-a>,<plugin-b>', 'Less.js Option: To specify plugins separated by commas.').option('--less-args <less-arg1>=<less-arg1-value>,<less-arg1>=<less-arg2-value>', 'Less.js Option: To specify any other less options e.g. \'--less-args math=strict,strict-units=on,include-path=.\/dir1\\;.\/dir2\'.').parse();

var programOption = program.opts();
...
fs.access(configPath, fs.constants.F_OK, function (err) {
  if (!err) {
    var _data = fs.readFileSync(configPath);
    var customConfig = JSON.parse(_data.toString());
